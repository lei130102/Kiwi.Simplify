...
{
    /// <summary>
    /// Combines color button functionality with the styling features of the Kiwi Toolkit.
	/// </summary>
    ...
    [Designer("Kiwi.ComponentFactory.Toolkit.KiwiColorButtonDesigner, Kiwi.ComponentFactory.Toolkit, Version=1.0.0.0, Culture=neutral, PublicKeyToken=360f304a9e10e704")]
	[ToolboxItem(true)]
    [DesignerCategory("code")]
    [Description("Raises an event when the user clicks it.")]
    //
    [ToolboxBitmap(typeof(KiwiColorButton), "ToolboxBitmaps.KiwiColorButton.bmp")]
    public class KiwiColorButton : VisualSimpleBase, IButtonControl, IContentValues
    {
        ...
        private KiwiContextMenuSeparator _separatorTheme, _separatorStandard, _separatorRecent;
        private KiwiContextMenuHeading _headingTheme, _headingStandard, _headingRecent;
        private KiwiContextMenuColorColumns _colorsTheme, _colorsStandard, _colorsRecent;
        private KiwiContextMenuSeparator _separatorNoColor;
        private KiwiContextMenuItems _itemsNoColor;
        private KiwiContextMenuItem _itemNoColor;
        private KiwiContextMenuSeparator _separatorMoreColors;
        private KiwiContextMenuItems _itemsMoreColors;
        private KiwiContextMenuItem _itemMoreColors;
        #endregion
        ...
        /// <summary>
        /// Occurs when the value of the KiwiCommand property changes.
        /// </summary>
        [Category("Property Changed")]
        [Description("Occurs when the value of the KiwiCommand property changes.")]
        public event EventHandler KiwiCommandChanged;
        ...
        #region Identity
        /// <summary>
        /// Initialize a new instance of the KiwiColorButton class.
		/// </summary>
        public KiwiColorButton()
        {
            ...
            _kiwiContextMenu = new KiwiContextMenu();
            _separatorTheme = new KiwiContextMenuSeparator();
            _headingTheme = new KiwiContextMenuHeading("Theme Colors");
            _colorsTheme = new KiwiContextMenuColorColumns(ColorScheme.OfficeThemes);
            _separatorStandard = new KiwiContextMenuSeparator();
            _headingStandard = new KiwiContextMenuHeading("Standard Colors");
            _colorsStandard = new KiwiContextMenuColorColumns(ColorScheme.OfficeStandard);
            _separatorRecent = new KiwiContextMenuSeparator();
            _headingRecent = new KiwiContextMenuHeading("Recent Colors");
            _colorsRecent = new KiwiContextMenuColorColumns(ColorScheme.None);
            _separatorNoColor = new KiwiContextMenuSeparator();
            _itemNoColor = new KiwiContextMenuItem("&No Color", Properties.Resources.ButtonNoColor, new EventHandler(OnClickNoColor));
            _itemsNoColor = new KiwiContextMenuItems();
            _itemsNoColor.Items.Add(_itemNoColor);
            _separatorMoreColors = new KiwiContextMenuSeparator();
            _itemMoreColors = new KiwiContextMenuItem("&More Colors...", new EventHandler(OnClickMoreColors));
            _itemsMoreColors = new KiwiContextMenuItems();
            _itemsMoreColors.Items.Add(_itemMoreColors);
            _kiwiContextMenu.Items.AddRange(new KiwiContextMenuItemBase[] { _separatorTheme, _headingTheme, _colorsTheme,
        ...
        [DefaultValue(null)]
        public virtual KiwiCommand KiwiCommand
        {
            get { return _command; }
            ...
            set
            {
                if (_command != value)
                {
                    ...
                    _command = value;
                    OnKiwiCommandChanged(EventArgs.Empty);
        ...
        {
            if (KiwiCommand != null)
                return KiwiCommand.ImageTransparentColor;
            else
        ...
        /// <summary>
        /// Raises the KiwiCommandChanged event.
        /// </summary>
        /// <param name="e">An EventArgs containing the event data.</param>
        protected virtual void OnKiwiCommandChanged(EventArgs e)
        {
            if (KiwiCommandChanged != null)
                KiwiCommandChanged(this, e);

            // Use the values from the new command
            if (KiwiCommand != null)
            {
                Enabled = KiwiCommand.Enabled;
                Values.Image = KiwiCommand.ImageSmall;
            }
        ...
        private bool ShowDropDown()
        {
            ...
            // Update the kiwi menu with this controls palette state
            if (_kiwiContextMenu != null)
            {
                if (PaletteMode != PaletteMode.Custom)
                    _kiwiContextMenu.PaletteMode = PaletteMode;
                else
                    _kiwiContextMenu.Palette = Palette;
            }
            ...
            if (!cpma.Cancel)
            {
                if (cpma.KiwiContextMenu != null)
                {
                    ...
                    if (CommonHelper.ValidKiwiContextMenu(cpma.KiwiContextMenu))
                    {
                        ...
                        {
                            case KiwiContextMenuPositionH.Before:
                                screenRect.X -= 1;
                                break;
                            case KiwiContextMenuPositionH.After:
                                screenRect.Width += 1;
                                break;
                        }
                        ...
                        HookContextMenuEvents(_kiwiContextMenu.Items, true);
                        ...
                        cpma.KiwiContextMenu.Show(this, screenRect, cpma.PositionH, cpma.PositionV);
                    }
                }
            }
        ...
        private KiwiContextMenuPositionH GetPositionH()
        {
            switch (DropDownOrientation)
            {
                ...
                case VisualOrientation.Bottom:
                case VisualOrientation.Top:
                    return KiwiContextMenuPositionH.Left;
                case VisualOrientation.Left:
                    return KiwiContextMenuPositionH.Before;
                case VisualOrientation.Right:
                    return KiwiContextMenuPositionH.After;
            }
        }

        private KiwiContextMenuPositionV GetPositionV()
        {
            switch (DropDownOrientation)
            {
                default:
                case VisualOrientation.Bottom:
                    return KiwiContextMenuPositionV.Below;
                case VisualOrientation.Top:
                    return KiwiContextMenuPositionV.Above;
                case VisualOrientation.Left:
                case VisualOrientation.Right:
                    return KiwiContextMenuPositionV.Top;
            }
        }
        ...
        private void OnKiwiContextMenuClosed(object sender, EventArgs e)
        {
            ...
            HookContextMenuEvents(_kiwiContextMenu.Items, false);
        }
        ...
        private void HookContextMenuEvents(KiwiContextMenuCollection collection, bool hook)
        {
            // Search for items of interest
            foreach (KiwiContextMenuItemBase item in collection)
            {
                // Hook into color events
                if (item is KiwiContextMenuColorColumns)
                {
                    KiwiContextMenuColorColumns columns = (KiwiContextMenuColorColumns)item;
        ...
        private void UpdateRecentColors(Color color)
        {
            ...
            if (AutoRecentColors)
            {
                ...
                foreach (KiwiContextMenuItemBase item in _kiwiContextMenu.Items)
                {
                    // Only interested in the non-recent colors color columns
                    if ((item is KiwiContextMenuColorColumns) && (item != _colorsRecent))
                    {
                        // Cast to correct type
                        KiwiContextMenuColorColumns colors = (KiwiContextMenuColorColumns)item;
        ...
        private void DecideOnVisible(KiwiContextMenuItemBase visible, KiwiContextMenuItemBase target)
        {
            bool previous = false;
            ...
            if (target.Visible)
            {
                ...
                foreach (KiwiContextMenuItemBase item in _kiwiContextMenu.Items)
                {
                    ...
                    if (!((item is KiwiContextMenuSeparator) ||
                          (item is KiwiContextMenuHeading)))
                    {
