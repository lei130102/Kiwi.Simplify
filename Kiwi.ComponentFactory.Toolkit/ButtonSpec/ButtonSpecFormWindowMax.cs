...
{
    /// <summary>
    /// Implementation for the fixed maximize button for kiwi form.
    /// </summary>
    public class ButtonSpecFormWindowMax : ButtonSpecFormFixed
    {
        ...
        public ButtonSpecFormWindowMax(KiwiForm form)
            ...
            if (GetViewEnabled())
            {
                ...
                if (!KiwiForm.InertForm)
                {
                    ...
                    if (GetView().ClientRectangle.Contains(mea.Location))
                    {
                        ...
                        if (KiwiForm.WindowState == FormWindowState.Maximized)
                            KiwiForm.SendSysCommand(PI.SC_RESTORE);
                        else
                            KiwiForm.SendSysCommand(PI.SC_MAXIMIZE);
