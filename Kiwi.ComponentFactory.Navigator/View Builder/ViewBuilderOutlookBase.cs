...
namespace Kiwi.ComponentFactory.Navigator
{
                                                     ...
                                                     ISeparatorSource
    {
        ...
        protected class PageToButtonEdge : Dictionary<KiwiPage, ViewDrawBorderEdge> { };
        ...
        protected class OutlookButtonSpecCollection : ButtonSpecCollection<ButtonSpec>
        {
            ...
            public OutlookButtonSpecCollection(KiwiNavigator owner)
                : base(owner)
            {
            }
            #endregion
        ...
        static ViewBuilderOutlookBase()
        {
            ...
            Stream streamBlueUp = myAssembly.GetManifestResourceStream("Kiwi.ComponentFactory.Navigator.Resources.BlueUp.bmp");
            Stream streamBlueDown = myAssembly.GetManifestResourceStream("Kiwi.ComponentFactory.Navigator.Resources.BlueDown.bmp");
        ...
        public bool SeparatorMoving(Point mouse, Point splitter)
        {
            ...
            if (mouseDelta > 0)
            {
                ...
                for (int i = Navigator.Pages.Count - 1; i >= 0; i--)
                {
                    ...
                    KiwiPage page = Navigator.Pages[i];
                    ...
                    if (_pageStackLookup.ContainsKey(page))
                    {
                        ...
                        if (checkButton.Visible)
                        {
                            ...
                            if (mouseDelta >= checkLength)
                            {
                                ...
                                checkButton.Page.SetFlags(KiwiPageFlags.PageInOverflowBarForOutlookMode);
                ...
                foreach (ViewDrawNavCheckButtonBase checkButton in _pageOverflowLookup.Values)
                    if (checkButton.Visible)
                    {
                        ...
                        if (mousePos < (separatorPos - checkButtonPos))
                        {
                            ...
                            checkButton.Page.ClearFlags(KiwiPageFlags.PageInOverflowBarForOutlookMode);
		...
		public override void Construct(KiwiNavigator navigator,
                                       ViewManager manager,
        ...
        /// <summary>
        /// Gets the KiwiPage associated with the provided view element.
        /// </summary>
        /// <param name="element">Element to search against.</param>
        /// <returns>Reference to KiwiPage; otherwise null.</returns>
        public override KiwiPage PageFromView(ViewBase element)
        {
            if (_pageOverflowLookup != null)
            {
                foreach (KeyValuePair<KiwiPage, ViewDrawNavCheckButtonBase> pair in _pageOverflowLookup)
            ...
            {
                foreach (KeyValuePair<KiwiPage, ViewDrawNavCheckButtonBase> pair in _pageStackLookup)
                    if (pair.Value == element)
                        return pair.Key;
            }
        ...
        public override void PageVisibleStateChanged(KiwiPage page)
        {
            ...
            {
                bool showPageStack = page.LastVisibleSet && !page.AreFlagsSet(KiwiPageFlags.PageInOverflowBarForOutlookMode);
        ...
        public override void PageEnabledStateChanged(KiwiPage page)
        {
            // Reflect new state in the check button
            UpdateStatePalettes();
        ...
        /// <summary>
        /// Notification that a kiwi page appearance property has changed.
        /// </summary>
        /// <param name="page">Page that has changed.</param>
        /// <param name="property">Name of property that has changed.</param>
        public override void PageAppearanceChanged(KiwiPage page, string property)
        {
        ...
        /// <summary>
        /// Notification that kiwi page flags have changed.
        /// </summary>
        /// <param name="page">Page that has changed.</param>
        /// <param name="changed">Set of flags that have changed value.</param>
        public override void PageFlagsChanged(KiwiPage page, KiwiPageFlags changed)
        {
            // Any change to the overflow bar setting requires a layout to effect
            if ((changed & KiwiPageFlags.PageInOverflowBarForOutlookMode) == KiwiPageFlags.PageInOverflowBarForOutlookMode)
                PerformNeedPaint(true);
        ...
        public override void Destruct()
        {
            ...
            _events = false;
            Navigator.Pages.Inserted -= new TypedHandler<KiwiPage>(OnPageInserted);
            Navigator.Pages.Removed -= new TypedHandler<KiwiPage>(OnPageRemoved);
        ...
        public override bool SelectNextPage(KiwiPage page,
                                            bool wrap,
                                            bool ctrlTab)
        {
        ...
        /// <param name="ctrlTab">Associated with a Ctrl+Tab action.</param>
        /// <returns>True if new page selected; otherwise false.</returns>
        public override bool SelectPreviousPage(KiwiPage page,
                                                bool wrap,
                                                bool ctrlTab)
        {
            // There must be at least one page and allowed to select a page
        ...
        public override void PerformNextAction(DirectionButtonAction action, KiwiPage page)
        {
            // Ask the header group to update the action
            action = _headerGroup.NextActionEnabled(action);
        ...
        /// <param name="action">Requested action.</param>
        /// <param name="page">Selected page at time of action request.</param>
        public override void PerformPreviousAction(DirectionButtonAction action, KiwiPage page)
        {
            // Ask the header group to update the action
            action = _headerGroup.PreviousActionEnabled(action);
        ...
        public string GetOverflowButtonStates()
        {
            ...
            if (_pageOverflowLookup != null)
            {
                // There swill be an overflow button per kiwi page
                foreach (KiwiPage page in Navigator.Pages)
                {
        ...
        public void UnshrinkAppropriatePages()
        {
            ...
            {
                // Make each visible kiwi page have its check button and border edge visible unless
        ...
        /// <returns></returns>
        protected virtual ViewDrawNavOutlookOverflow CreateOverflowItem(KiwiPage page,
        ...
        protected virtual void ReorderCheckButtonsOverflow(KiwiPage page,
        ...
        protected virtual void BringPageIntoView(KiwiPage page)
        {
        ...
        protected KiwiPage FirstOutlookActionPage()
        {
        ...
        /// </summary>
        /// <returns>Page for selection; otherwise null.</returns>
        protected KiwiPage LastOutlookActionPage()
        {
        ...
        protected KiwiPage NextOutlookActionPage(KiwiPage page)
        {
            bool found = false;
        ...
        /// <param name="page">Current page to work from.</param>
        /// <returns>New page that should be selected.</returns>
        protected KiwiPage PreviousOutlookActionPage(KiwiPage page)
        {
            bool found = false;
        ...
        private void CreateStackItems()
        {
            ...
            // Create a check button to represent each kiwi page
            foreach (KiwiPage page in Navigator.Pages)
            {
                // Create the draw view element for the check button and provide page it represents
                ...
                bool showPage = page.LastVisibleSet && !page.AreFlagsSet(KiwiPageFlags.PageInOverflowBarForOutlookMode);
        ...
        private void OnPageInserted(object sender, TypedCollectionEventArgs<KiwiPage> e)
        {
            if (!Navigator.IsDisposed && _events)
            {
                ...
                // Can we show the page as a stacking item?
                bool showPageStack = e.Item.LastVisibleSet && !e.Item.AreFlagsSet(KiwiPageFlags.PageInOverflowBarForOutlookMode);
        ...
        private bool PageInTheStack(KiwiPage page)
        {
        ...
        private void OnDropDownClick(object sender, EventArgs e)
        {
            if (!Navigator.InternalDesignMode)
            {
                ...
                ResetCachedKiwiContextMenu();

                // Add the three standard entries
                KiwiContextMenuItem moreButtons = new KiwiContextMenuItem(Navigator.Outlook.TextMoreButtons, _moreButtons, new EventHandler(OnShowMoreClick));
                KiwiContextMenuItem fewerButtons = new KiwiContextMenuItem(Navigator.Outlook.TextFewerButtons, _fewerButtons, new EventHandler(OnShowFewerClick));
                KiwiContextMenuItem addRemoveButtons = new KiwiContextMenuItem(Navigator.Outlook.TextAddRemoveButtons);
                KiwiContextMenuItems addRemoveButtonItems = new KiwiContextMenuItems();
                ...
                _kcm.Items.Add(new KiwiContextMenuItems(new KiwiContextMenuItemBase[] { moreButtons, fewerButtons, addRemoveButtons }));
                ...
                foreach (KiwiPage page in Navigator.Pages)
                {
                    ...
                    KiwiContextMenuItem pageMenuItem = new KiwiContextMenuItem(page.GetTextMapping(Navigator.Button.ContextMenuMapText),
        ...
        private void OnShowMoreClick(object sender, EventArgs e)
        {
            ...
            if (checkButton != null)
            {
                ...
                foreach (KiwiPage page in _pageOverflowLookup.Keys)
                    if (_pageOverflowLookup[page] == checkButton)
                    {
                        // Remove the flag that places this page on the overflow bar
                        page.ClearFlags(KiwiPageFlags.PageInOverflowBarForOutlookMode);
        ...
        private void OnShowFewerClick(object sender, EventArgs e)
        {
            ...
            foreach (ViewBase child in _viewLayout.Reverse())
                if (child.Visible && (child is ViewDrawNavOutlookStack))
                {
                    ...
                    // Search for the page that is represented by this check button
                    foreach (KiwiPage page in _pageStackLookup.Keys)
                        if (_pageStackLookup[page] == checkButton)
                        {
                            // Set the flag that places this page on the overflow bar
                            page.SetFlags(KiwiPageFlags.PageInOverflowBarForOutlookMode);
                            return;
                        }
                }
        ...
        private void OnCheckButtonDragRect(object sender, ButtonDragRectangleEventArgs e)
        {
            ...
            e.PreDragOffset = (Navigator.AllowPageReorder && reorderItem.Page.AreFlagsSet(KiwiPageFlags.AllowPageReorder));
                ...
                if (childView.Visible && childView.ClientRectangle.Contains(e.PointOffset))
                {
                    ...
                    if (childView != reorderView)
                    {
                        ...
                        if (foundReorderView)
                        {
                            ...
                            {
                                KiwiPage movePage = PageFromView(reorderView);
                                KiwiPage targetPage = PageFromView(childView);
