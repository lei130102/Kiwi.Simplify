...
{
    /// <summary>
    /// Extends the KiwiWorkspace with common functionality shared by various docking implementations.
    /// </summary>
    public abstract class KiwiSpace : KiwiWorkspace
    {
        #region Type Declarations
        /// <summary>
        /// Lookup between a cell and the cell state.
        /// </summary>
        protected class CellToCachedState : Dictionary<KiwiWorkspaceCell, CachedCellState> { };
        ...
        #region Events
        /// <summary>
        /// Occurs when the focus moves to be inside the KiwiWorkspaceCell instance.
        /// </summary>
        [Category("DockableWorkspace")]
        [Description("Occurs when the focus moves to be inside the KiwiWorkspaceCell instance.")]
        public event EventHandler<WorkspaceCellEventArgs> CellGainsFocus;

        /// <summary>
        /// Occurs when the focus moves away from inside the KiwiWorkspaceCell instance.
        /// </summary>
        [Category("DockableWorkspace")]
        [Description("Occurs when the focus moves away from inside the KiwiWorkspaceCell instance.")]
        ...
        #region Identity
        /// <summary>
        /// Initialize a new instance of the KiwiSpace class.
        /// </summary>
        /// <param name="storeName">Name to associate with store pages.</param>
        public KiwiSpace(string storeName)
        {
        ...
        public override void WritePageElement(XmlWriter xmlWriter, KiwiPage page)
        {
        ...
        public override KiwiPage ReadPageElement(XmlReader xmlReader,
                                                    string uniqueName,
                                                    UniqueNameToPage existingPages)
        {
            ...
            if (page != null)
            {
                ...
                if (CommonHelper.StringToBool(CommonHelper.XmlAttributeToText(xmlReader, "S")))
                    page = new KiwiStorePage(page.UniqueName, _storeName);
                else
                {
        ...
        protected override void NewCellInitialize(KiwiWorkspaceCell cell)
        {
            ...
            if (ApplyDockingAppearance)
            {
                ...
                cell.Header.HeaderValuesPrimary.MapImage = MapKiwiPageImage.None;
            ...
            cell.TabDoubleClicked += new EventHandler<KiwiPageEventArgs>(OnCellTabDoubleClicked);
            cell.TabVisibleCountChanged += new EventHandler(OnCellTabVisibleCountChanged);
            cell.Pages.Inserting += new TypedHandler<KiwiPage>(OnCellPagesInserting);
            ...
            if (cell.Pages.Count > 0)
                for (int i = cell.Pages.Count - 1; i >= 0; i--)
                    OnCellPageInserting(new KiwiPageEventArgs(cell.Pages[i], i));
        }
        ...
        protected override void ExistingCellDetach(KiwiWorkspaceCell cell)
        {
        ...
        protected virtual void UpdateCellActions(KiwiWorkspaceCell cell, CachedCellState cellState)
        {
            if (ApplyDockingDropDownAction)
            {
                ...
                if (cellState.DropDownButtonSpec == null)
                {
                    ...
                    cellState.DropDownButtonSpec.KiwiContextMenu = new KiwiContextMenu();
                    cellState.DropDownButtonSpec.KiwiContextMenu.Opening += new CancelEventHandler(OnCellDropDownOpening);
                ...
                if (cell.SelectedPage == null)
                    ...
                    cellState.DropDownButtonSpec.Visible = cell.SelectedPage.AreFlagsSet(KiwiPageFlags.DockingAllowDropDown);
            }
            ...
            if (ApplyDockingPinAction)
            {
                ...
                {
                    if (AutoHiddenHost)
                        cellState.PinButtonSpec.Visible = cell.SelectedPage.AreFlagsSet(KiwiPageFlags.DockingAllowDocked);
                    else
                        cellState.PinButtonSpec.Visible = cell.SelectedPage.AreFlagsSet(KiwiPageFlags.DockingAllowAutoHidden);
                }
            }
            ...
            if (ApplyDockingCloseAction)
            {
                ...
                if (cell.SelectedPage == null)
                    ...
                    cellState.CloseButtonSpec.Visible = cell.SelectedPage.AreFlagsSet(KiwiPageFlags.DockingAllowClose);
            }
        }
        #endregion
        ...
        private void OnCellPrimaryHeaderRightClicked(object sender, EventArgs e)
        {
            ...
            if (ApplyDockingAppearance)
            {
                ...
                if (cell.SelectedPage != null)
                {
                    ...
                    if (!args.Cancel && CommonHelper.ValidKiwiContextMenu(args.KiwiContextMenu))
                        args.KiwiContextMenu.Show(this, Control.MousePosition);
                }
            }
        }
        ...
        private void OnCellPrimaryHeaderDoubleClicked(object sender, EventArgs e)
        {
            ...
            if (ApplyDockingAppearance)
            {
                ...
                foreach (KiwiPage page in cell.Pages)
                    if (page.LastVisibleSet && !(page is KiwiStorePage))
        ...
        private void OnCellTabDoubleClicked(object sender, KiwiPageEventArgs e)
        {
        ...
        private void OnCellSelectedPageChanged(object sender, EventArgs e)
        {
            if (ApplyDockingCloseAction || ApplyDockingPinAction)
            {
                ...
                if (cellState.SelectedPage != null)
                    cellState.SelectedPage.FlagsChanged -= new KiwiPageFlagsEventHandler(OnCellSelectedPageFlagsChanged);
        ...
        private void OnCellSelectedPageFlagsChanged(object sender, KiwiPageFlagsEventArgs e)
        {
            if (ApplyDockingCloseAction || ApplyDockingPinAction)
            {
                // Only need to process the change in flags if the page in question is a selected page
                KiwiPage page = (KiwiPage)sender;
                KiwiWorkspaceCell cell = CellForPage(page);
        ...
        private void OnCellDropDownOpening(object sender, CancelEventArgs e)
        {
            if (ApplyDockingDropDownAction)
            {
                ...
                foreach (CachedCellState cellState in _lookupCellState.Values)
                    if ((cellState.DropDownButtonSpec != null) && (cellState.DropDownButtonSpec.KiwiContextMenu == kcm))
                    {
                        if (cellState.Cell.SelectedPage != null)
                            OnPageDropDownClicked(new CancelDropDownEventArgs(cellState.DropDownButtonSpec.KiwiContextMenu, cellState.Cell.SelectedPage));
        ...
        private void OnCellPagesInserting(object sender, TypedCollectionEventArgs<KiwiPage> e)
        {
