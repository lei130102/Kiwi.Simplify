...
namespace Kiwi.ComponentFactory.Docking
{
                                           ...
                                           IDockingElement
    {
        ...
        public virtual void PropogateAction(DockingPropogateAction action, KiwiPage[] pages)
        {
            // Propogate the action request to all the child elements
            // (use reverse order so if element removes itself we still have a valid loop)
            for (int i = Count - 1; i >= 0; i--)
        ...
        public virtual KiwiPage PropogatePageState(DockingPropogatePageState state, string uniqueName)
        {
            // Search all child docking elements
            for (int i = 0; i < Count; i++)
            {
                // If the child knows the answer then return it now
                KiwiPage page = this[i].PropogatePageState(state, uniqueName);
                if (page != null)
                    return page;
            }
        ...
        public virtual void PropogatePageList(DockingPropogatePageList state, KiwiPageCollection pages)
        {
            // Propogate the action request to all the child elements
            // (use reverse order so if element removes itself we still have a valid loop)
            for (int i = Count - 1; i >= 0; i--)
        ...
        public virtual void PropogateCellList(DockingPropogateCellList state, KiwiWorkspaceCellList cells)
        {
            // Propogate the action request to all the child elements
            // (use reverse order so if element removes itself we still have a valid loop)
            for (int i = Count - 1; i >= 0; i--)
        ...
        public virtual void PropogateDragTargets(KiwiFloatingWindow floatingWindow,
        ...
        public virtual KiwiDockingFloating FindDockingFloating(string uniqueName)
        {
            // Default to not finding the element
            KiwiDockingFloating floatingElement = null;
        ...
        public virtual KiwiDockingEdgeDocked FindDockingEdgeDocked(string uniqueName)
        {
            // Default to not finding the element
            KiwiDockingEdgeDocked edgeDockedElement = null;
        ...
        public virtual KiwiDockingEdgeAutoHidden FindDockingEdgeAutoHidden(string uniqueName)
        {
            // Default to not finding the element
            KiwiDockingEdgeAutoHidden edgeAutoHiddenElement = null;
        ...
        public virtual KiwiDockingWorkspace FindDockingWorkspace(string uniqueName)
        {
            // Default to not finding the element
            KiwiDockingWorkspace workspaceElement = null;
        ...
        public virtual KiwiDockingNavigator FindDockingNavigator(string uniqueName)
        {
            // Default to not finding the element
            KiwiDockingNavigator navigatorElement = null;
        ...
        public virtual void LoadElementFromXml(XmlReader xmlReader, KiwiPageCollection pages)
        {
        ...
        /// </summary>
        public void DemandPagesNotBePresent(KiwiPage[] pages)
        {
            ...
            foreach (KiwiPage page in pages)
                if (!(page is KiwiStorePage) && DockingManager.ContainsPage(page))
        ...
        {
            if (!HasDockManager)
                throw new ApplicationException("Cannot perform this operation when there is no access to a KiwiDockingManager.");
        }
        ...
        /// <summary>
        /// Finds the KiwiDockingManager instance that owns this part of the docking hieararchy.
        /// </summary>
        [Browsable(false)]
        public KiwiDockingManager DockingManager
        {
            get
            {
                ...
                while (parent != null)
                {
                    ...
                    if (parent is KiwiDockingManager)
                        return parent as KiwiDockingManager;
        ...
        protected virtual void LoadDockingElement(XmlReader xmlReader, KiwiPageCollection pages)
        {
        }
