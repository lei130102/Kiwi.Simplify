...
{
    /// <summary>
    /// Provides docking functionality within a control edge using a KiwiDockingSpace.
    /// </summary>
    [ToolboxItem(false)]
    [DesignerCategory("code")]
    [DesignTimeVisible(false)]
    public class KiwiDockingDockspace : KiwiDockingSpace
    {
            ...
            : base(name, "Docked")
        {
            ...
            DockspaceControl.CellPageInserting += new EventHandler<KiwiPageEventArgs>(OnSpaceCellPageInserting);
        ...
        {
            get { return (KiwiDockspace)SpaceControl; }
        }
        ...
        public KiwiDockingEdgeAutoHidden EdgeAutoHiddenElement
        {
            get
            {
                ...
                if (dockingEdge != null)
                {
                    ...
                    return dockingEdge["AutoHidden"] as KiwiDockingEdgeAutoHidden;
                }
                    ...
                    if (value == Order)
                    {
                        ...
                        if (parent != null)
                        {
                            ...
                            for (int i = parent.Controls.Count - 1; i >= 0; i--)
                            {
                                ...
                                if ((c is KiwiAutoHiddenPanel) || (c is KiwiAutoHiddenSlidePanel))
                                    indexInsert = i;

                                // Insert before the 'order' found dockspace separator (this handles the Order>0 cases)
                                if (c is KiwiDockspaceSeparator)
                                {
                                    if (value == 1)
                                    {
                                        indexInsert = i - 1;
                                                  ...
                                                  DragTargetList targets)
        {
            if (DockspaceControl.CellVisibleCount > 0)
            {
                ...
                if (pages.Count > 0)
                {
                    DragTargetList dockspaceTargets = DockspaceControl.GenerateDragTargets(new PageDragEndData(this, pages), KiwiPageFlags.DockingAllowDocked);
        ...
        {
            KiwiPage page = DockspaceControl.PageForUniqueName(uniqueName);
        ...
        public override void SaveElementToXml(XmlWriter xmlWriter)
        {
            ...
            if (parent != null)
            {
                // Count the number of KiwiDockspace that occur after ourself in the collection by scanning
                ...
                for (int i = parent.Controls.Count - 1; i >= 0; i--)
                {
                    ...
                    if (child == DockspaceControl)
                        break;
                    else if (child is KiwiDockspace)
                        numDockspace++;
                }
        ...
        private void OnDockspaceBeforePageDrag(object sender, PageDragCancelEventArgs e)
        {
            ...
            foreach (KiwiPage page in e.Pages)
                if (!(page is KiwiStorePage) && (DockspaceControl.CellForPage(page) != null))
                    pages.Add(page);
