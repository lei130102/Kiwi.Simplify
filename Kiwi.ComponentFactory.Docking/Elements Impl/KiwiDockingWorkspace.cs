...
{
    /// <summary>
    /// Provides docking functionality by attaching to an existing KiwiDockableWorkspace
    /// </summary>
    [ToolboxItem(false)]
    [DesignerCategory("code")]
    [DesignTimeVisible(false)]
    public class KiwiDockingWorkspace : KiwiDockingSpace
    {
        #region Identity
        /// <summary>
        /// Initialize a new instance of the KiwiDockingWorkspace class.
        /// </summary>
        /// <param name="name">Initial name of the element.</param>
        public KiwiDockingWorkspace(string name)
            : this(name, "Workspace", new KiwiDockableWorkspace())
        {
        }
        ...
        public KiwiDockingWorkspace(string name,
                                       string storeName,
                                       KiwiDockableWorkspace workspace)
            : base(name, storeName)
        {
            if (workspace == null)
            ...
            DockableWorkspaceControl.CellPageInserting += new EventHandler<KiwiPageEventArgs>(OnSpaceCellPageInserting);
        ...
        {
            get { return (KiwiDockableWorkspace)SpaceControl; }
        }
                                                  ...
                                                  DragTargetList targets)
        {
            ...
            foreach (KiwiPage page in dragData.Pages)
                if (page.AreFlagsSet(KiwiPageFlags.DockingAllowWorkspace))
                    pages.Add(page);

            // Do we have any pages left for dragging?
            if (pages.Count > 0)
            {
                DragTargetList workspaceTargets = DockableWorkspaceControl.GenerateDragTargets(new PageDragEndData(this, pages), KiwiPageFlags.DockingAllowWorkspace);
        ...
        {
            KiwiPage page = DockableWorkspaceControl.PageForUniqueName(uniqueName);
        ...
        private void OnDockableWorkspaceBeforePageDrag(object sender, PageDragCancelEventArgs e)
        {
            ...
            foreach (KiwiPage page in e.Pages)
                if (!(page is KiwiStorePage) && (DockableWorkspaceControl.CellForPage(page) != null))
                    pages.Add(page);
