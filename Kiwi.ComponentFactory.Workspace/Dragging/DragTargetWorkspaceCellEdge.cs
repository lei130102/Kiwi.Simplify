...
namespace Kiwi.ComponentFactory.Workspace
{
    ...
    public class DragTargetWorkspaceCellEdge : DragTargetWorkspaceEdge
    {
        ...
        public override bool IsMatch(Point screenPt, PageDragEndData dragEndData)
        {
            ...
            if (_visibleNotDraggedPages == -1)
            {
                ...
                if (dragEndData.Navigator == _cell)
                {
                    ...
                    KiwiPageCollection visiblePages = new KiwiPageCollection();
                    foreach (KiwiPage page in _cell.Pages)
                        if (page.LastVisibleSet)
                            visiblePages.Add(page);

                    // Remove all those that are being dragged
                    foreach (KiwiPage page in dragEndData.Pages)
        ...
        public override bool PerformDrop(Point screenPt, PageDragEndData data)
        {
            ...
            KiwiWorkspaceSequence parent = Cell.WorkspaceParent as KiwiWorkspaceSequence;
            if (parent != null)
            {
                // Transfer the dragged pages into a new cell
                KiwiWorkspaceCell cell = new KiwiWorkspaceCell();
                KiwiPage page = ProcessDragEndData(Workspace, cell, data);
                ...
                else
                {
                        ...
                        (!dropHorizontal && (parent.Orientation == Orientation.Horizontal)))
                    {
                        ...
                        KiwiWorkspaceSequence sequence = new KiwiWorkspaceSequence(sequenceOrientation);
