...
namespace Kiwi.ComponentFactory.Ribbon
{
    ...
    [Designer("Kiwi.ComponentFactory.Ribbon.KiwiGalleryDesigner, Kiwi.ComponentFactory.Ribbon, Version=1.0.0.0, Culture=neutral, PublicKeyToken=360f304a9e10e704")]
    [DesignerCategory("code")]
    [Description("Select from a group of possible images.")]
    //
    [ToolboxBitmap(typeof(KiwiGallery), "ToolboxBitmaps.KiwiGallery.bmp")]
    public class KiwiGallery : VisualSimpleBase
    {
        #region Instance Fields
        private KiwiRibbon _ribbon;
        private KiwiGalleryRangeCollection _dropButtonRanges;
        ...
        private KiwiContextMenu _dropMenu;
        ...
        #region Identity
        /// <summary>
        /// Initialize a new instance of the KiwiGallery class.
		/// </summary>
        public KiwiGallery()
        {
            // Defaults
            ...
            _dropButtonRanges = new KiwiGalleryRangeCollection();
        ...
        public KiwiGalleryRangeCollection DropButtonRanges
        {
                                           ...
                                           int actualLineItems)
        {
            ...
            {
                KiwiContextMenuImageSelect imageSelect = new KiwiContextMenuImageSelect();
            ...
            {
                foreach (KiwiGalleryRange range in _dropButtonRanges)
                {
                    // If not the first item in the menu, add a separator
                    if (_dropMenu.Items.Count > 0)
                        _dropMenu.Items.Add(new KiwiContextMenuSeparator());
            ...
            if (!args.Cancel && CommonHelper.ValidKiwiContextMenu(args.KiwiContextMenu))
            {
                ...
                foreach (KiwiContextMenuItemBase item in _dropMenu.Items)
                    if (item is KiwiContextMenuImageSelect)
                    {
                        KiwiContextMenuImageSelect itemSelect = (KiwiContextMenuImageSelect)item;
                ...
                args.KiwiContextMenu.Closed += new ToolStripDropDownClosedEventHandler(OnDropMenuClosed);

                // Remember the delegate we need to fire when the menu is dismissed
                _finishDelegate = finishDelegate;

                // Show the menu to the user
                args.KiwiContextMenu.Show(this, screenRect, hPosition, vPosition);
            }
            else
            {
        ...
        {
            KiwiContextMenuImageSelect imageSelect = (KiwiContextMenuImageSelect)sender;
